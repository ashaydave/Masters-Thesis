...
public class RunYOLO8n : MonoBehaviour
{
    ...
    public PdPatchManager pdPatchManager;
    ...
    public void DrawBox(BoundingBox box , int id)
    {
        ...
        SendClassifiedString(box.label);
    }
    ...
    void SendClassifiedString(string classifiedString)
    {
        if (pdPatchManager != null)
        {
            pdPatchManager.HandleClassifiedObject(classifiedString);
        }

        else
        {
            Debug.LogWarning("PdPatchManager script not assigned!");
        }
    }
